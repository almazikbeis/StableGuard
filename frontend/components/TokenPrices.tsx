...
export function TokenPrices({ tokens, fetchedAt }: Props) {
  ...
  return (
    <div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100">
              <th className="text-left pb-2 text-xs font-medium text-gray-500 pr-4">Token</th>
              <th className="text-right pb-2 text-xs font-medium text-gray-500 pr-4">Price</th>
              <th className="text-right pb-2 text-xs font-medium text-gray-500 pr-4">Confidence</th>
              <th className="text-right pb-2 text-xs font-medium text-gray-500">vs USDC</th>
            </tr>
          </thead>
          <tbody>
            {tokens.map((t) => (
              <tr key={t.symbol} className="border-b border-gray-50 last:border-0">
                <td className="py-2.5 pr-4">
                  <div className="flex items-center gap-2">
                    <span className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center text-[10px] font-bold text-gray-600">
                      {t.symbol.slice(0, 2)}
                    </span>
                    <div>
      ...
      {time && (
        <p className="text-xs text-gray-400 mt-3 text-right">Updated {time}</p>
      )}
    </div>
  );
}
